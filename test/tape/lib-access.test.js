...
test('lib/access.js', function (t) {
    ...
    t.test('Should create and access to some non-existent properties.', function (
        st) {
        ...
        var newObject = access(object, keys, function (lastObject, lastKey,
            ...
            st.isNot(this, object,
                'The object didn\'t mutate since `mutate` was a falsy value.');
            st.deepEquals(lastObject, {},
                'Keys with undefined values weren\'t added.');
            st.is(lastKey, 'c',
                'The handler is called one time (at the end).');
            st.is(exists, false,
                'Some property doesn\'t exist.');
        ...
        st.deepEquals(newObject, Object.assign({'a': {'b': {'c': 3}}}, object),
