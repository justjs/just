...
test('/lib/on', function (t) {
	...
	t.test('Should set multiple events.', {'timeout': 3000}, function (st) {
		...
		on('html, body', eventTypes, onActive, options);
	});

	t.test('Should add event listeners to any element.', {'timeout': 3000},
		function (st) {
	...
	t.test('Should add event listeners to multiple elements.', {'timeout': 3000},
		function (st) {
