...
test('lib/check.js', function (t) {
    ...
    t.test('Should check if the given value looks like the others.', function (st) {

        st.is(check({}, null, [], function () {}), false, 'Neither is `{}`.');
        st.is(check({}, null, [], {}), true, 'Last value is `{}`');
        st.is(check(null, {}, [], 'null'), false, 'Neither is `null`.');
        st.is(check(null, null), true, 'Both are `null`.');
        st.is(check(void 0, undefined), true, 'Both are `undefined`.');
        st.is(check(1, NaN), true, 'Both are Numbers.');
        st.is(check(function () {}, Object), true, 'Both are Functions.');
        st.is(check({'a': 1}, 0, 0, 0, {}), true, 'Last value is `{}`.');
    ...
    t.test('Should throw when the given value does not "check" ' +
        ...
        st.doesNotThrow(function () {
            ...
            st.is(check.throwable(0, 5), 0, 'The first value is returned.');
